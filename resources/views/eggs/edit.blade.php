    ...
    <form method="POST" action="{{ route('eggs.update', $egg) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="crates" value="{{ old('crates', $egg->crates) }}" step="0.01" min="0"
                    class="w-full border rounded p-2 dark:bg-gray-800 dark:border-gray-600 dark:text-white" required>
                @error('crates')
                    <p class="text-red-600 dark:text-red-400 text-sm">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="additional_eggs" value="{{ old('additional_eggs', $egg->additional_eggs) }}" min="0" max="29"
                    class="w-full border rounded p-2 dark:bg-gray-800 dark:border-gray-600 dark:text-white" required>
            ...
            <div>
                <label class="block text-gray-700 dark:text-gray-300">
                    <input type="checkbox" name="is_cracked" value="1" {{ old('is_cracked', $egg->is_cracked) ? 'checked' : '' }}
