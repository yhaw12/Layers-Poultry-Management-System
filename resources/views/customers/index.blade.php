...
<div class="container mx-auto px-4 py-8 space-y-12 bg-gray-100 dark:bg-[#0a0a23] dark:text-white">
    ...
    <section class="flex justify-between items-center">
        <h2 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400">Customers</h2>
        <a href="{{ route('customers.create') }}" 
           ...
           aria-label="Add new customer">
            <span class="mr-2" aria-hidden="true">➕</span> Add Customer
        </a>
    </section>
    ...
    <section>
        <div class="bg-gradient-to-r from-white to-gray-100 dark:from-[#1a1a3a] dark:to-gray-800 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
            ...
            <form method="GET" action="{{ route('customers.index') }}" class="flex flex-wrap items-end gap-4" id="filter-form">
    ...
    <section>
        <div class="bg-gradient-to-r from-white to-gray-100 dark:from-[#1a1a3a] dark:to-gray-800 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
            ...
            <div id="customers-table-wrapper" aria-live="polite">
                        ...
                        <div class="text-center py-12">
                            ...
                            <a href="{{ route('customers.create') }}" 
                               ...
                               aria-label="Add your first customer">
                                <span class="mr-2" aria-hidden="true">➕</span> Add Your First Customer
                            </a>
                        </div>
                    @else
                        <!-- Desktop Table -->
                                    ...
                                    @foreach ($customers as $customer)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                                            ...
                                            <td class="p-4 flex space-x-2">
                                                <button type="button" data-id="{{ $customer->id }}" data-url="{{ route('customers.edit', $customer) }}" 
                                                        class="edit-btn inline-flex items-center px-3 py-1 bg-blue-500 text-white rounded-lg shadow hover:bg-blue-600 text-xs focus:ring-2 focus:ring-blue-500 transition" 
                                                        aria-label="Edit customer {{ $customer->name }}">
                                                    <span class="mr-2" aria-hidden="true">✏️</span> Edit
                                                </button>
                                                <button type="button" data-id="{{ $customer->id }}" data-url="{{ route('customers.destroy', $customer) }}" 
                            ...
                            @foreach ($customers as $customer)
                                <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                                    <div class="flex justify-between items-center mb-2">
                                        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $customer->name }}</h4>
                            ...
                            @if ($customers->hasPages())
                                <div class="flex space-x-2">
                                    <a href="{{ $customers->previousPageUrl() }}" 
                                       class="inline-flex items-center px-4 py-2 bg-gray-600 text-white text-sm font-medium rounded-md hover:bg-gray-700 dark:bg-gray-500 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 transition {{ $customers->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}"
                                       aria-label="Previous page" {{ $customers->onFirstPage() ? 'disabled' : '' }} data-ajax="true">
                                    ...
                                    <a href="{{ $customers->nextPageUrl() }}" 
                                       class="inline-flex items-center px-4 py-2 bg-gray-600 text-white text-sm font-medium rounded-md hover:bg-gray-700 dark:bg-gray-500 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 transition {{ !$customers->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}"
                                       aria-label="Next page" {{ !$customers->hasMorePages() ? 'disabled' : '' }} data-ajax="true">
                                        Next
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40" aria-modal="true" role="dialog">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg max-w-lg w-full p-4">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-2">Are you sure you want to delete this customer? This action cannot be undone.</p>
    ...
    <div id="edit-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40" aria-modal="true" role="dialog">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg max-w-lg w-full p-4">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-2">Are you sure you want to edit this customer?</p>
            <div class="mt-4 flex justify-end gap-2">
                ...
                <button id="edit-confirm" class="px-4 py-2 rounded bg-blue-500 text-white text-sm font-medium hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition" aria-label="Confirm edit">
                    <span class="flex items-center">
...
(function () {
    ...
    async function extractFragmentFromHtml(htmlText, selector = '#customers-table-wrapper') {
    ...
    async function reloadTable(url = null) {
        ...
        try {
            ...
            const newInner = await extractFragmentFromHtml(text, '#customers-table-wrapper');
            if (!newInner) {
    ...
    function initTableControls() {
        // Delete buttons
        document.querySelectorAll('#customers-table-wrapper .delete-btn').forEach(btn => {
            btn.removeEventListener('click', deleteBtnClickHandler);
            btn.addEventListener('click', deleteBtnClickHandler);
        });

        // Edit buttons
        document.querySelectorAll('#customers-table-wrapper .edit-btn').forEach(btn => {
            btn.removeEventListener('click', editBtnClickHandler);
            btn.addEventListener('click', editBtnClickHandler);
        });

        // Pagination links
        document.querySelectorAll('#customers-table-wrapper [data-ajax="true"]').forEach(link => {
    ...
    deleteConfirm.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const msg = data ? (data.message ?? 'Customer deleted') : 'Customer deleted';
                if (ok) {
                    toast(msg, 'success', 2000);
                    await reloadTable();
                } else {
                    toast(data.message || 'Failed to delete customer', 'error', 3000);
                }
            } else {
    ...
    filterForm.addEventListener('submit', async (e) => {
        ...
        const url = '{{ route('customers.index') }}' + (query ? '?' + query : '');
