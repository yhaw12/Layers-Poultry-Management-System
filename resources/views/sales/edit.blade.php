{{-- sales.edit.blade.php (update form section) --}}
@extends('layouts.app')
...
@section('content')
    <div class="bg-white dark:bg-[#1a1a3a] p-6 rounded-2xl shadow-md">
        <form method="POST" action="{{ route('sales.update', $sale) }}" class="space-y-6">
            ...
            <div>
                <label for="saleable_type" class="block text-gray-7
            ...
            <div>
                ...
                <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $sale->due_date ? $sale->due_date->format('Y-m-d') : now()->addDays(7)->format('Y-m-d')) }}" class="w-full border rounded p-2 dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                @error('due_date')
                    <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                @endError
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    populateSaleableOptions('{{ old('saleable_type', $sale->saleable_type) }}');
