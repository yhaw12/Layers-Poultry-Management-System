...
<div class="container mx-auto px-4 py-8 space-y-12 bg-gray-100 dark:bg-[#0a0a23] dark:text-white">
    ...
    <section>
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Edit Employee</h2>
        ...
        <div class="bg-white dark:bg-[#1a1a3a] p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 max-w-md mx-auto">
            <form method="POST" action="{{ route('employees.update', $employee) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input name="name" type="text" id="name" value="{{ old('name', $employee->name) }}" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('name') border-red-500 @enderror" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input name="telephone" type="text" id="telephone" value="{{ old('telephone', $employee->telephone) }}" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('telephone') border-red-500 @enderror" maxlength="20">
                    @error('telephone')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @endError
                </div>
