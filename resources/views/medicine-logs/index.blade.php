...
<div class="container mx-auto px-4 py-8 space-y-12 bg-gray-100 dark:bg-[#0a0a23] dark:text-white">
    ...
    <section class="flex justify-between items-center">
        <h2 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400">Medicine Logs</h2>
        <a href="{{ route('medicine-logs.create') }}" 
           ...
           aria-label="Add new medicine log">
            <span class="mr-2" aria-hidden="true">➕</span> Add Medicine Log
        </a>
    </section>
    ...
    <section>
        <div class="bg-gradient-to-r from-white to-gray-100 dark:from-[#1a1a3a] dark:to-gray-800 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
            ...
            <div id="medicine-logs-table-wrapper" aria-live="polite">
                @if ($medicineLogs->isEmpty())
                    <div class="text-center py-12">
                        <p class="text-gray-600 dark:text-gray-400 mb-4">No medicine logs found yet.</p>
                        <a href="{{ route('medicine-logs.create') }}" 
                           class="inline-flex items-center bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 focus:ring-2 focus:ring-blue-500 transition" 
                           aria-label="Add your first medicine log">
                            <span class="mr-2" aria-hidden="true">➕</span> Add Your First Medicine Log
                        </a>
                    </div>
                @else
                    <!-- Desktop Table -->
                        ...
                        <table class="w-full border-collapse rounded-lg overflow-hidden text-sm">
                            <thead>
                                <tr class="bg-gray-200 dark:bg-gray-700">
                                    <th scope="col" class="p-4 text-left font-medium text-gray-700 dark:text-gray-200 uppercase tracking-wider">Medicine</th>
                                    ...
                                    <th scope="col" class="p-4 text-left font-medium text-gray-700 dark:text-gray-200 uppercase tracking-wider">Unit</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                @foreach ($medicineLogs as $log)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                                        ...
                                        <td class="p-4 text-gray-700 dark:text-gray-300">{{ $log->date ? $log->date->format('Y-m-d') : 'N/A' }}</td>
                                        <td class="p-4 text-gray-700 dark:text-gray-300">{{ $log->notes ?? 'None' }}</td>
                                        <td class="p-4 flex space-x-2">
                                            {{-- <a href="{{ route('medicine-logs.show', $log) }}" 
                                               ...
                                               aria-label="View medicine log for {{ $log->medicine_name }} on {{ $log->date ? $log->date->format('Y-m-d') : 'N/A' }}">
                                                <span class="mr-2" aria-hidden="true">👀</span> View
                                            </a> --}}
                                            <button type="button" data-id="{{ $log->id }}" data-url="{{ route('medicine-logs.edit', $log) }}" 
                                                    ...
                                                    aria-label="Edit medicine log for {{ $log->medicine_name }} on {{ $log->date ? $log->date->format('Y-m-d') : 'N/A' }}">
                                                <span class="mr-2" aria-hidden="true">✏️</span> Edit
                                            </button>
                                            <button type="button" data-id="{{ $log->id }}" data-url="{{ route('medicine-logs.destroy', $log) }}" 
                                                    ...
                                                    aria-label="Delete medicine log for {{ $log->medicine_name }} on {{ $log->date ? $log->date->format('Y-m-d') : 'N/A' }}">
                        ...
                        @foreach ($medicineLogs as $log)
                            <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                                <div class="flex justify-between items-center mb-2">
                                    <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $log->medicine_name }}</h4>
                                ...
                                <div class="grid grid-cols-1 gap-2 text-sm text-gray-700 dark:text-gray-300">
                                    <div><strong>Type:</strong> {{ ucfirst($log->type) }}</div>
                                    <div><strong>Quantity:</strong> {{ number_format($log->quantity, 2) }}</div>
                                    <div><strong>Unit:</strong> {{ $log->unit }}</div>
                                    <div><strong>Date:</strong> {{ $log->date ? $log->date->format('Y-m-d') : 'N/A' }}</div>
                                    <div><strong>Notes:</strong> {{ $log->notes ?? 'None' }}</div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <!-- Pagination -->
                    @if ($medicineLogs instanceof \Illuminate\Pagination\LengthAwarePaginator && $medicineLogs->hasPages())
                        <div class="mt-6 flex justify-between items-center">
                            <div class="flex space-x-2">
                                <a href="{{ $medicineLogs->previousPageUrl() }}" 
                                   class="pagination-link inline-flex items-center px-4 py-2 bg-gray-600 text-white text-sm font-medium rounded-md hover:bg-gray-700 dark:bg-gray-500 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 transition {{ $medicineLogs->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}" 
                                   aria-label="Previous page" {{ $medicineLogs->onFirstPage() ? 'disabled' : '' }} data-ajax="true">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                                    </svg>
                                    Previous
                                </a>
                                <a href="{{ $medicineLogs->nextPageUrl() }}" 
                                   class="pagination-link inline-flex items-center px-4 py-2 bg-gray-600 text-white text-sm font-medium rounded-md hover:bg-gray-700 dark:bg-gray-500 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 transition {{ !$medicineLogs->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}" 
                                   aria-label="Next page" {{ !$medicineLogs->hasMorePages() ? 'disabled' : '' }} data-ajax="true">
                                    Next
                            ...
                            <span class="text-sm text-gray-600 dark:text-gray-400">
                                Page {{ $medicineLogs->currentPage() }} of {{ $medicineLogs->lastPage() }}
                            </span>
                        </div>
                    @endif
                @endif
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40" aria-modal="true" role="dialog">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg max-w-lg w-full p-4">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-2">Are you sure you want to delete this medicine log? This action cannot be undone.</p>
    ...
    <div id="edit-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40" aria-modal="true" role="dialog">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg max-w-lg w-full p-4">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-2">Are you sure you want to edit this medicine log?</p>
            <div class="mt-4 flex justify-end gap-2">
...
(function () {
    ...
    async function extractFragmentFromHtml(htmlText, selector = '#medicine-logs-table-wrapper') {
    ...
    async function reloadTable(url = null) {
        ...
        try {
            ...
            const wrapper = document.getElementById('medicine-logs-table-wrapper');
    ...
    function initTableControls() {
        // Delete buttons
        document.querySelectorAll('#medicine-logs-table-wrapper .delete-btn').forEach(btn => {
            btn.removeEventListener('click', deleteBtnClickHandler);
            btn.addEventListener('click', deleteBtnClickHandler);
        });

        // Edit buttons
        document.querySelectorAll('#medicine-logs-table-wrapper .edit-btn').forEach(btn => {
            btn.removeEventListener('click', editBtnClickHandler);
            btn.addEventListener('click', editBtnClickHandler);
        });

        // Pagination links
        document.querySelectorAll('#medicine-logs-table-wrapper .pagination-link').forEach(link => {
    ...
    deleteConfirm.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const msg = data ? (data.message ?? 'Medicine log deleted') : 'Medicine log deleted';
                if (ok) {
                    toast(msg, 'success', 2000);
                    await reloadTable();
                } else {
                    toast(data.message || 'Failed to delete medicine log', 'error', 3000);
                }
            } else {
