...
<div class="container mx-auto px-4 py-8 space-y-12 bg-gray-100 dark:bg-[#0a0a23] dark:text-white">
    ...
    <section>
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Add Medicine Log</h2>
    ...
    <section>
        <div class="bg-white dark:bg-[#1a1a3a] p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 max-w-md mx-auto">
            <form method="POST" action="{{ route('medicine-logs.store') }}" class="space-y-6">
                @csrf
                <div>
                    <label for="medicine_name" class="block text-gray-700 dark:text-gray-300">Medicine Name</label>
                    <input name="medicine_name" type="text" id="medicine_name" value="{{ old('medicine_name') }}" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('medicine_name') border-red-500 @enderror" required>
                    @error('medicine_name')
                ...
                <div>
                    ...
                    <select name="type" id="type" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('type') border-red-500 @enderror" required>
                        ...
                        <option value="consumption" {{ old('type') == 'consumption' ? 'selected' : '' }}>Consumption</option>
                    </select>
                    @error('type')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @endError
                ...
                <div>
                    ...
                    <input name="unit" type="text" id="unit" value="{{ old('unit') }}" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('unit') border-red-500 @enderror" placeholder="e.g., ml, mg, tablets" required>
                    @error('unit')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @endError
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('notes') border-red-500 @enderror" rows="4">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @endError
                </div>
