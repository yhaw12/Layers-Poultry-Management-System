        ...
        <div class="bg-white dark:bg-[#1a1a3a] p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 max-w-md mx-auto">
            <form method="POST" action="{{ route('mortalities.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input name="cause" type="text" id="cause" value="{{ old('cause') }}" class="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white @error('cause') border-red-500 @enderror">
                    @error('cause')
                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                    @endError
                </div>
