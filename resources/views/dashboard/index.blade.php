...
@push('scripts')
    <script>
        ...
        @if (isset($eggTrend) && $eggTrend->isNotEmpty())
            createChart('eggTrend', 'eggChartType', {{ $eggTrend->toJson() }}, {
                type: 'line',
                title: 'Egg Production Trend',
                yAxis: 'Crates',
                xAxis: 'Date',
                data: {
                    labels: {!! json_encode($eggTrend->pluck('date')->toArray()) !!},
                    datasets: [{
                        label: 'Egg Crates',
                        data: {!! json_encode($eggTrend->pluck('value')->toArray()) !!},
                        borderColor: '#4F46E5',
        ...
        @if (isset($feedTrend) && $feedTrend->isNotEmpty())
            createChart('feedTrend', 'feedChartType', {{ $feedTrend->toJson() }}, {
                type: 'line',
                title: 'Feed Consumption Trend',
                yAxis: 'Quantity (kg)',
                xAxis: 'Date',
                data: {
                    labels: {!! json_encode($feedTrend->pluck('date')->toArray()) !!},
                    datasets: [{
                        label: 'Feed Quantity',
                        data: {!! json_encode($feedTrend->pluck('value')->toArray()) !!},
                        borderColor: '#10B981',
        ...
        @if (isset($salesTrend) && $salesTrend->isNotEmpty())
            createChart('salesTrend', 'salesChartType', {{ $salesTrend->toJson() }}, {
                type: 'line',
                title: 'Sales Trend',
                yAxis: 'Amount ($)',
                xAxis: 'Date',
                data: {
                    labels: {!! json_encode($salesTrend->pluck('date')->toArray()) !!},
                    datasets: [{
                        label: 'Sales Amount',
                        data: {!! json_encode($salesTrend->pluck('value')->toArray()) !!},
                        borderColor: '#059669',
        ...
        // Income Trend Chart
        @if (isset($incomeLabels) && !empty($incomeLabels))
            createChart('incomeChart', 'incomeChartType', {{ $incomeTrend->toJson() }}, {
                type: 'line',
                title: 'Income Trend',
                yAxis: 'Amount ($)',
                xAxis: 'Date',
                data: {
                    ...
                    datasets: [{
                        label: 'Income',
                        data: {!! json_encode($incomeTrend->pluck('value')->toArray()) !!},
                        borderColor: '#34D399',
        ...
        @if (isset($invoiceStatuses) && array_sum($invoiceStatuses) > 0)
            createChart('invoiceStatus', 'invoiceChartType', {{ json_encode($invoiceStatuses) }}, {
                type: 'bar',
                title: 'Invoice Status Distribution',
                yAxis: 'Count',
                xAxis: 'Status',
                data: {
                    labels: {!! json_encode(array_keys($invoiceStatuses)) !!},
                    datasets: [{
                        label: 'Invoices',
                        data: {!! json_encode(array_values($invoiceStatuses)) !!},
                        backgroundColor: ['#EF4444', '#10B981', '#F59E0B', '#DC2626'],
                        borderColor: ['#EF4444', '#10B981', '#F59E0B', '#DC2626'],
                        borderWidth: 1
                    }]
                }
            });
        ...
        @if (isset($expenseTrend) && $expenseTrend->isNotEmpty())
            createSparklineChart('expensesTrend', {{ $expenseTrend->toJson() }}, '#EF4444');
        @endif
        @if (isset($incomeTrend) && $incomeTrend->isNotEmpty())
            createSparklineChart('incomeTrend', {{ $incomeTrend->toJson() }}, '#10B981');
        @endif
        @if (isset($profitTrend) && $profitTrend->isNotEmpty())
            createSparklineChart('profitTrend', {{ $profitTrend->toJson() }}, '#059669');
        @endif

        // alerts
        document.addEventListener('DOMContentLoaded', () => {
            // Handle Dismiss All button with AJAX
            const dismissAllForm = document.getElementById('dismiss-all-form');
            const dismissAllBtn = document.getElementById('dismiss-all-btn');
            if (dismissAllForm && dismissAllBtn) {
                dismissAllForm.addEventListener('submit', async (e) => {
                    e.preventDefault();
                    dismissAllBtn.disabled = true;
                    dismissAllBtn.textContent = 'Dismissing...';
                    dismissAllBtn.classList.add('opacity-50', 'cursor-not-allowed');

                    try {
                        const response = await fetch(dismissAllForm.action, {
                            method: 'POST',
                            headers: {
                        ...
                        if (response.ok) {
                            ...
                            if (data.success) {
                                ...
                                const alertItems = alertsContent.querySelectorAll('.list-item');
                                alertItems.forEach(item => {
                                    item.classList.add('opacity-0');
                                    setTimeout(() => item.remove(), 300);
                                });
                                setTimeout(() => {
                                    alertsContent.innerHTML = '<p class="no-data text-gray-500 dark:text-gray-400 italic text-center py-4">No active alerts at this time.</p>';
                                    document.getElementById('alerts-section').style.display = 'none';
                                    notificationManager.show(Date.now(), 'All alerts dismissed successfully.', 'success', 3000);
                                    // Redirect to dashboard
                                    window.location.href = '{{ route('dashboard') }}';
                                }, 300);
                            } else {
                                throw new Error(data.error || 'Failed to dismiss alerts');
                            }
                        } else {
                        ...
                        notificationManager.show(Date.now(), 'Failed to dismiss alerts.', 'critical', 5000);
                        dismissAllBtn.disabled = false;
                        dismissAllBtn.textContent = 'Dismiss All';
                        dismissAllBtn.classList.remove('opacity-50', 'cursor-not-allowed');
                    }
                });
            }

            // Handle individual Mark as Read with AJAX
            document.querySelectorAll('.dismiss-single-form').forEach(form => {
                form.addEventListener('submit', async (e) => {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            const data = await response.json();
                            if (data.success) {
                                const listItem = form.closest('.list-item');
                                listItem.classList.add('opacity-0');
                                setTimeout(() => {
                                    listItem.remove();
                                    const remainingAlerts = document.querySelectorAll('.list-item').length;
                                    if (remainingAlerts === 0) {
                                        document.getElementById('alerts-content').innerHTML = '<p class="no-data text-gray-500 dark:text-gray-400 italic text-center py-4">No active alerts at this time.</p>';
                                        document.getElementById('alerts-section').style.display = 'none';
                                        // Redirect to dashboard if no alerts remain
                                        window.location.href = '{{ route('dashboard') }}';
                                    }
                                    notificationManager.show(Date.now(), 'Alert marked as read.', 'success', 3000);
                                }, 300);
                            } else {
                        ...
                        notificationManager.show(Date.now(), 'Failed to mark alert as read.', 'critical', 5000);
